...
const CosmeticsBOM = ({ skus, setSkus, activeRoute, setActiveRoute, selectedSkuId, setSelectedSkuId }: {
    ...
    const [newSkuData, setNewSkuData] = useState({ id: '', name: '', category: '세럼/앰플' });
    ...
    const handleCreateSku = () => {
        ...
        setIsCreating(false);
        setNewSkuData({ id: '', name: '', category: '세럼/앰플' });
        setPendingBom([]);
    };
    ...
    const FIXED_CATEGORIES = ['세럼/앰플', '크림', '토너/스킨', '클렌저/바디', '기타1', '기타2'];
                                    ...
                                    {FIXED_CATEGORIES.map(cat => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                className="bg-white p-8 rounded-[2rem] border border-[#EBE5DF] shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all cursor-pointer group relative overflow-hidden"
                                            >
                                                <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                                                    <Folder className="w-16 h-16 text-[#8C6D58]" />
                                                </div>

                                                <div className="relative z-10">
                                                    <div className="flex justify-between items-start mb-6">
                                                        <div className="p-3 bg-[#FDFBF9] rounded-2xl border border-[#EBE5DF] group-hover:border-[#8C6D58]/30 transition-colors">
                                                            <Folder className="w-6 h-6 text-[#8C6D58]" />
                                                        </div>
                                                        <span className="px-3 py-1 bg-[#F5F1EB] text-[#7D7673] text-[10px] font-black rounded-full uppercase tracking-widest">Category</span>
                                                    </div>

                                                    <h3 className="text-2xl font-black text-[#2C2A29] mb-2">{cat}</h3>
                                                    <div className="flex items-center justify-between mb-6">
                                                        <p className="text-[#7D7673] font-bold text-sm">등록 제품 {catSkus.length}건</p>
                                                        <div className="flex items-center gap-1.5">
                                                            <div className="w-2 h-2 rounded-full bg-emerald-500"></div>
                                                            <span className="text-xs font-black text-[#476652]">BOM 완료 {analyzedCount}건</span>
                                                        </div>
                                                    </div>

                                                    <div className="space-y-2">
                                                        <div className="flex justify-between text-[10px] font-black text-[#A8A19D] uppercase tracking-tighter">
                                                    ...
                                                    <div className="mt-8 flex justify-between items-center text-[#8C6D58] font-black text-sm opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all">
                                                        <span>현황 상세보기</span>
