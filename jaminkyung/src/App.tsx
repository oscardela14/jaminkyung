...
import LogisticsClosing from './LogisticsClosing';

function App() {
  const [activeRoute, setActiveRoute] = useState('partner-schedule');
  ...
  const [isEtcExpanded, setIsEtcExpanded] = useState(true);
  ...
  const renderContent = () => {
    ...
    if (activeRoute === 'logistics-closing') return <LogisticsClosing />;
  ...
  const isLogisticsActive = activeRoute === 'inventory' || activeRoute === 'logistics' || activeRoute === 'logistics-closing';
        ...
        <nav className="flex-1 px-3 py-4 flex flex-col gap-1 overflow-y-auto">
          ...
          {isLogisticsExpanded && (
            <div className="flex flex-col gap-1 px-2 py-1">
              ...
              {subBtn('logistics-closing', <Calculator className="w-4 h-4 shrink-0" />, '물류 마감품의')}
            </div>
          )}
